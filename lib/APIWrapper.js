    ...
    function Wrapper(tcm_feed, adobe_feed) {
      ...
      var extra_folio_methods = {
        ...
        "get_welcome_imgs": function() {
          if (this.tcm) {
            return [
              this.tcm.intro_img ? settings.tcmfeed_image_root+this.tcm.intro_img : null, 
              settings.tcmfeed_image_root+this.tcm.welcome_img
            ];
          }
          else {
            return [null, this.get_cover_img()]
          }
        },
      ...
      that.libraryService.get_visible = function() {
        ...
        var now = moment(),
            ...
            latest_folio = _.max(folios, function(folio) { return moment(folio.publicationDate) });
        ...
        folios = _.chain(folios)
          .map(function(folio) {
            ...
            return wrapped; 
          }).sortBy(function(folio1, folio2) {
            var priority_comp = comp(priority(folio2), priority(folio1));
            if (priority_comp === 0) {
              return comp(moment(folio2.publicationDate), moment(folio1.publicationDate));
            }
