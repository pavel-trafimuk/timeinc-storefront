(function() {
  ...
  window.APIWrapper = function(raw_api, cb) {
    ...
    function Wrapper(tcm_feed, adobe_feed) {
      ...
      // LIBRARY METHODS ///////////////////////////////////////////////
      that.libraryService._update_if_empty = function() {
        if (this.currentTransaction) {
          return transaction_to_deferred(this.currentTransaction);
        }
        else if (_.size(this.folioMap) == 0) {
          return transaction_to_deferred(this.updateLibrary());
        }
        else {
