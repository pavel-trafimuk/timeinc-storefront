  ...
  window.APIWrapper = function(raw_api, cb) {
    ...
    function Wrapper(tcm_feed, adobe_feed) {
      ...
      var extra_folio_methods = {
        ...
        "view_or_preview": function(opts) {
          ...
          if (folio.isDownloadable) folio.download_and_view(opts);          
            ...
            if (folio.currentStateChangingTransaction() != null) {
              ...
              return folio.currentStateChangingTransaction().completedSignal.addOnce(arguments.callee);
            }
            var transaction;
              ...
              try_to_view(function() {
                ...
                if (folio.canDownloadContentPreview()) {
                  ...
                  App.error("NoContentPreview", 
                    ...
                    + " | currentStateChangingTransaction: " + folio.currentStateChangingTransaction()
                  );
        ...
        "purchase_and_download": function(opts) {
          ...
          function download() {
            if (folio.isDownloadable || folio._get_current_download()) {
              dl_transaction = folio._download();
              ...
              dl_transaction.stateChangedSignal.add(function() {
                (opts.download_progress || $.noop)(dl_transaction.progress);
      ...
      that.authenticationService._get_lucie_data = function() {
        var d = arguments.callee.d = new $.Deferred();
        if (!this.isUserAuthenticated) {
          this.lucie_data = null;
          this.lucie_subscriber = {id: null, type: 'dsc', is_active: false};
          update_omniture_subscriber();
          d.resolve(this.lucie_subscriber);
          return;
        }

        lucieApi._default_api_args.authToken = this.token;

        lucieApi.entitlements(function(data) {
          that.authenticationService.lucie_data = data;
          that.authenticationService.lucie_subscriber = {
            id: $("subscriberId", data).text(),
            type: $("subscriberType", data).text(),
            is_active: $("subscriberIsActive", data).text() == "Y"
          };
          update_omniture_subscriber();
          d.resolve(that.authenticationService.lucie_subscriber);
        });
      }
    ...
    function load_adobe_feed(cb) {
      ...
      $.get(settings.adobeFeedUrl, {t: ts}, success, "xml")
        .fail(function() {
          if (DEBUG) $.get(settings.adobeFeedUrl_dev, {t: ts}, success, "xml");
        });
    } 
    function ensure_folios_are_loaded(cb) {
      ...
      var t = raw_api.libraryService.currentTransaction,
          retry = _.partial(arguments.callee, cb),
          on_sc = function(t) {
            // if one of our transactions errors out, punt
            if (t.state < 0 && t.error) {
              console.log("Transaction Error in APIWrapper.js:ensure_folios_are_loaded() with " + t.jsonClassName);
              console.log("error: " + t.error + " " + t.error.code);
              _.delay(location.reload, 1500, true);
            }
          };

      if (t) {
        // a transaction is already in progress
        t.completeSignal.add(retry);
        t.stateChangedSignal.add(_.partial(on_sc, t));
      }
      else if (_.size(raw_api.libraryService.internal) == 0) {
        // there are no folios... we'd better update the library
        t = raw_api.libraryService.updateLibrary();
        t.completedSignal.add(function() {
         if (_.size(raw_api.libraryService.internal) == 0) cb(null);
         else retry(); 
        });
        t.stateChangedSignal.add(_.partial(on_sc, t));
      }
