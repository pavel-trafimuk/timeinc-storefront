(function() {
  ...
  window.APIWrapper = function(raw_api, cb) {
    ...
    function Wrapper(tcm_feed, adobe_feed) {
      ...
      that.authenticationService.get_lucie_data();
      ...
      that.authenticationService.user_is_subscriber = function(cb) {
        ...
        // if we don't have this data loaded yet, wait 50ms and try again
        if (this.lucie_subscriber === undefined) {
          setTimeout(function() { 
            that.authenticationService.user_is_subscriber(cb);
          }, 50);
          return;
        }
        ...
        if (this.lucie_subscriber.is_active) {
          is_sub = true;
        }
        cb(is_sub);
      }
