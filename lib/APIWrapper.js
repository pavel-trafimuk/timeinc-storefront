...
(function() {
  ...
  window.APIWrapper = function(raw_api, cb) {
    ...
    function Wrapper(tcm_feed, adobe_feed) {
      ...
      _(tcm_feed.slideshows).each(function(slideshow) {
        tcm_feed[slideshow] = slideshow;
      });
      ...
      var extra_folio_methods = {
        ...
        "folioDateForAmex": function() {
          function is_coverdate(s) {
            var has_year = s.match(/\b\d{4}\b/) !== null,
                has_month = s.match(/january|february|march|april|may|june|july|august|september|october|november|december/i) !== null;
            return has_year && has_month;
          }
          if (is_coverdate(this.title)) return this.title;
          else return this.folioNumber;
        },
        ...
        "get_html_description": function() {
          if (this.tcm && this.tcm.post_description_HTML) {
            return this.tcm.post_description_HTML;
          }
          return this.adb.find("description").html() || this.description;
        },
        ...
        "get_coverdate": function() {
          var d = moment(this.adb.find("coverDate").text());
          if (!d) {
            return this.get_publicationdate();
          }
          return d;
        },
      ...
      // get slideshow from tcm feed
      that.libraryService.get_slideshow_data_for_slide_index = function(i) {
        if (!that.tcm_feed || !that.tcm_feed.slideshows) return [];
        var ss = that.tcm_feed.slideshows[0];
        return [
          (ss.slides || [])[i],
          (ss.actions || [])[i],
          (ss.params || [])[i] || []
      ...
      that.libraryService.get_store_hero_slideshow = function() {
          if (!that.tcm_feed || !that.tcm_feed.slideshows) return [];
          return _.map(that.tcm_feed.slideshows[0].slides, tcmimg, function(img) {
            return img;
          });
      };
    ...
    function load_tcm_feed(cb) {
      function success(data) {
        console.log("tcm feed loaded");
        localStorage.tcmfeed_image_root = settings.tcmfeed_image_root;
        cb(null, data)
      }
    ...
    var before_async_t = t();
    async.parallel([
        cache_async("tcmfeed", 15, load_tcm_feed, json_codec),
        cache_async("adobefeed", 15, load_adobe_feed, xml_codec),
        ensure_folios_are_loaded
      ],
      function(err, results) {
        console.log("load feeds time - " + (t() - before_async_t));
        if (window.DEBUG) {
          $("targetDimensions", results[1]).text(window.screen.height + "x" + window.screen.width);
        }
        console.log("done loading stuff for wrapper");
        if (!settings.tcmfeed_image_root && localStorage.tcmfeed_image_root) {
          settings.tcmfeed_image_root = localStorage.tcmfeed_image_root;
        }
