...
window.libBanner = (function() {
  ...
  function mk_dimensions_array() {
    var result = {};

    for (var i=arguments.length; i--;) {
      for (var j = arguments[i].length; j--;) {
        result[ arguments[i][j] ] = arguments[i];
      }
    }
    return result;
  }
  var screen_W = window.screen.width,  // get device screen width
      ...
      dimensionsArrays = mk_dimensions_array(
        // ipad dimensions
        ["2048x1536", "1024x768", "1024x748", "2048x1496"],

        // iphone dimensions
        ["1136x640", "568x320", "960x640", "480x320"]
      ),
      dimensionsArray = dimensionsArrays[screen_H + "x" + screen_W],
  ...
  function cache_async(cache_key, duration_minutes, fn, codec) {
    ...
    if (cache_async.mem_cache[cache_key]) {
      return function(cb) { cb(cache_async.mem_cache[cache_key]) }
    }
    ...
    return function(cb) {
      ...
      fn(function(data) {
        ...
        cache_async.mem_cache[cache_key] = data;

        cb(data);
      });
    }
  }
  cache_async.mem_cache = {};
  ...
  var get_tcm_data = function(cb) {
    ...
    $.getJSON(settings.PRODUCTION_TCM_FEED, {t: ts}, cb);
  }
    ...
    function done() {
      ...
      get_dps_data_map(function(dps_map) {
        get_tcm_data_map(function(tcm_map) {
          ...
          // find latest folio
          var latest_folio_id,
              latest_folio_pubdate;
          folio_ids.forEach(function(folio_id) {
            var pubdate;
            if (_.values(settings.preview_issue_product_ids).indexOf(folio_id) >= 0) {
              pubdate = -Infinity;
            }
            else {
              pubdate = get_pubdate(folio_id);
            }

            if (pubdate > latest_folio_pubdate) {
              latest_folio_id = folio_id;
              latest_folio_pubdate = pubdate;
            }
          });
