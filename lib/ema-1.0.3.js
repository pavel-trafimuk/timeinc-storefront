...
// eMagazines EMA-JS SDK.
// (c) 2013, eMagazines. All Rights Reserved.
//
...
var __defaultDuplicateTimeout = 60; // number of seconds duplicate transaction cache maintains records. 0 = no cache
var __duplicateDuplicateTransactionPurgeTimeSec = 3888000; // 45-days in secs - transaction records past this amt get flushed.
...
__ema.cache.lat = 0;
__ema.cache.lng = 0;
__ema.cache.isAdvertisingTrackingEnabled = null;
__ema.cache.identifierForVendor = null;
__ema.cache.advertisingIdentifier = null;
...
__ema.cache.duplicateFoundCallback = null;
__ema.cache.internalFlushCallback = null;
__ema.cache.eventSuccessCallback = null;
__ema.cache.eventErrorCallback = null;
__ema.cache.internalEventSuccessCallback = null;
__ema.cache.internalEventErrorCallback = null;
__ema.cache.flushMaxRec = 100;
__ema.cache.maxDuplicateTimeout = __defaultDuplicateTimeout;
__ema.appId = null;
__ema.token = null;
...
// Utility routine to create a hashcode from a string
//
...
__ema.cache.open = function () {
    ...
    database.transaction(function(d){
        d.executeSql("CREATE TABLE IF NOT EXISTS transactions (transid INTEGER PRIMARY KEY AUTOINCREMENT, " +
            "hashvalue INTEGER, transvalue TEXT, transdate INTEGER)", [],
            function(d, result) {
                d.executeSql("CREATE TABLE IF NOT EXISTS events (eventid INTEGER PRIMARY KEY AUTOINCREMENT, " +
                    "emid CHAR(64) UNIQUE, payload TEXT)", [],
                    function(d, result) {
        ...
        //
        // We auto-purge transactions older than N seconds.
        //
        purgeSql = "DELETE FROM transactions WHERE datetime(transdate / 1000, 'unixepoch') < datetime('now', '-"
            + __duplicateDuplicateTransactionPurgeTimeSec + " second')";
        d.executeSql(purgeSql);
    });
}
...
// Used to manage the transaction cache. We check to see if we've already seen this transaction in the
// last N days. If so we don't queue up the request

__ema.cache.checkAgainstTransactionCache = function(request, notfoundhandler, foundhandler) {
    var database = __ema.cache.db;
    var hash = request.hashCode();
    database.readTransaction(function(d) {
        d.executeSql("SELECT transid, transvalue, transdate from transactions where hashvalue=?", [hash],
            function(d, r) {
                if (r.rows.length > 0) {
                    var result = r.rows.item(0);
                    if (result) {
                        var tid = result.transid;
                        var tvalue = result.transvalue;
                        var tdate = result.transdate;

                        // If it exists we do a second check against the serialized transaction
                        // also if the request was sent more than N days ago.
                        //
                        if (request == tvalue) {
                            var now = new Date();
                            var old = new Date(tdate);
                            diffsecs = (now - old) / 1000.0;
                            if ((__ema.cache.maxDuplicateTimeout >= 0) && (diffsecs < __ema.cache.maxDuplicateTimeout)) {
                                if (foundhandler()) {
                                    foundhandler();
                                }
                            } else {
                                database.transaction(function(d){
                                        d.executeSql("DELETE from transactions WHERE transid = ?", [tid]);
                                    },
                                    null,
                                    function() {
                                        if (notfoundhandler) {
                            ...
                            // Found but new request is not the same as cached request
                            if (notfoundhandler) {
                                notfoundhandler();
                            }
                        }
...
__ema.cache.internalAddEvent = function (action, eventname, data, successhandler, failurehandler) {
    var localData = {};

    if (successhandler)  {
        __ema.cache.minFlushDelta = 0;     // Turn off the flush timer
        __ema.cache.internalEventSuccessCallback = function(action, eventname, emid) {
            __ema.cache.flushNow(__ema.cache.flushMaxRec, successhandler, failurehandler);
        }
    } else {
        __ema.cache.minFlushDelta = __minFlushDelta;    // we flush no more than every X seconds
        __ema.cache.internalEventSuccessCallback = null;
    ...
    // If adding event failed, we don't bother flushing
    if (failurehandler) {
        __ema.cache.minFlushDelta = 0;     // Turn off the flush timer
        __ema.cache.internalEventErrorCallback = function(action, eventname, error) {
            failurehandler(action, eventname, error);
        }
    }
    ...
    // We take a snapshot of the data before date/time values have been added to it, then compare it against
    // the transaction cache. The callback is called if the check
    //
    var localSnapshot = JSON.stringify(localData).replace(/\s/g, "");

    __ema.cache.checkAgainstTransactionCache(localSnapshot,
        // This is called if the snapshot is not in the transaction cache -- in which case it is also saved.
        function() {
            var now = new Date();
            ...
            database.transaction(function(d) {
                d.executeSql("INSERT INTO events (emid, payload) VALUES (?, ?);", [emid, jsondata],
                    function(d, result){
                        d.executeSql("UPDATE metadata SET highesteventid = (SELECT eventid FROM events WHERE rowid = ?)",
                            [result.insertId],
                        function(d, result) {
                            d.executeSql("INSERT into transactions (hashvalue, transvalue, transdate) VALUES (?, ?, ?);",
                                [ localSnapshot.hashCode(), localSnapshot, now ],
                                function(d, result) {
                                    if (__ema.cache.internalEventSuccessCallback) {
                                        __ema.cache.internalEventSuccessCallback(action, eventname, emid);
                                    }
                                ...
                                function(error) {
                                    if (__ema.cache.internalEventErrorCallback) {
                                        __ema.cache.internalEventErrorCallback(action, eventname, error);
                                    }
            ...
            function(error) { // If any step in the transaction fails
                if (__ema.cache.internalEventErrorCallback) {
                    __ema.cache.internalEventErrorCallback(action, eventname, error);
                }
                if (__ema.cache.eventErrorCallback) {
    ...
    // This is called if the entry was found in the transaction duplicate cache
    //
    function() {
        if (__ema.cache.duplicateFoundCallback) {
            __ema.cache.duplicateFoundCallback(action, eventname);
        }
    }
    );
}
...
__ema.cache.resetData = function () {
    var database = __ema.cache.db;
    database.transaction(function(d){
        d.executeSql("DELETE from transactions");
...
__ema.cache.setDuplicateFoundCallback = function (callback) {
    __ema.cache.duplicateFoundCallback = callback;
}
...
// IDFA support -- only available inside a DPS container v29 or above
// Info here: http://www.adobe.com/devnet/digitalpublishingsuite/articles/new-consumer-marketing-apis.html
//
    var idfaId = null;
    var idfaenabled = null;
    var idfavendor = null;

    if (typeof adobeDPS !== 'undefined') {
        if (typeof adobeDPS.deviceService !== 'undefined') {
            if (adobeDPS.deviceService.isAdvertisingTrackingEnabled != null) {
                enabled = adobeDPS.deviceService.isAdvertisingTrackingEnabled;
                if (enabled) {
                    idfaenabled = "Y";
                } else {
                    idfaenabled= "N";
                ...
                $.extend(__jsonData, {
                    adid_enabled: idfaenabled
                });
            }

            if (adobeDPS.deviceService.advertisingIdentifier != null) {
                idfaid = adobeDPS.deviceService.advertisingIdentifier;
                if (idfaid) {
                    $.extend(__jsonData, {
                        adid: idfaid
                    });
            ...
            if (adobeDPS.deviceService.identifierForVendor != null) {
                idfavendor = adobeDPS.deviceService.advertisingIdentifier;
                if (idfavendor) {
                    $.extend(__jsonData, {
                        advendor: idfavendor
                    });
...
// NOTE FROM JAMES (at Time Inc): THIS WAS MODIFIED FROM THE ORIGINAL 
// ema-*.js to work with the v2.5 storefront
function __eMagsInitHandlerWrapper() {
  if (!settings.eMagsAppId) return;
  return App.eMagsInit.apply(App, arguments);
}

function __scanForScriptAttributes (em) {
    em.appId = settings.eMagsAppId;
    em.initHandler = "__eMagsInitHandlerWrapper";
    // em.cache.maxDuplicateTimeout = 60 * 60 * 24 * 30;
}
...
function EMEvent (eventname, dict, successhandler, failurehandler) {
    var __e = __sharedEMA();
    __e.cache.internalAddEvent("event", eventname, dict, successhandler, failurehandler);
    return __e;
}

function EMUserData (email, phone, name, zipcode, extras, successhandler, failurehandler) {
    var __e = __sharedEMA();
    var mydata = {};
    if (email)
        mydata["_em"] = email;
    ...
    __e.cache.internalAddEvent("userdata", null, mydata, successhandler, failurehandler);
    return __e;
}

function EMPurchase (name, sku, price, currency, count, category, extras, successhandler, failurehandler) {
    var __e = __sharedEMA();
    var mydata = {};
    if (name)
        mydata["_pn"] = name;
    ...
    __e.cache.internalAddEvent("purchase", name, mydata, successhandler, failurehandler);
    return __e;
}

function EMCrossSell (newAppId, successhandler, failurehandler) {
    var __e = __sharedEMA();
    var mydata = {};
    if (newAppId) {
        mydata["_cs"] = newAppId;
        __e.cache.internalAddEvent("crosssell", null, mydata, successhandler, failurehandler);
    }
    return __e;
}

function EMCustomEvent (eventname, extras, successhandler, failurehandler) {
    var __e = __sharedEMA();
    var mydata = {};
    // If extras is a dict, we merge it in with this one otherwise we add it in as an item
    if (typeof(extras) === 'object') {
    ...
    __e.cache.internalAddEvent("customevent", eventname, mydata, successhandler, failurehandler);
    return __e;
}

function EMReferrer(referrerid, campaignid, extras, successhandler, failurehandler) {
    var __e = __sharedEMA();
    var mydata = {};
    if (referrerid)
    ...
    __e.cache.internalAddEvent("referrer", null, mydata, successhandler, failurehandler);
    return __e;
}

function EMError (error, extras, successhandler, failurehandler) {
    var __e = __sharedEMA();
    var mydata = {};
    if (error)
        mydata["_er"] = error;
    ...
    __e.cache.internalAddEvent("error", null, mydata, successhandler, failurehandler);
    return __e;
}
...
function EMSetDuplicateFoundCallback (callback)
{
    var __e = __sharedEMA();
    __e.cache.setDuplicateFoundCallback(callback);
}
...
// Given the current token it returns the campaign ID associated with it.
// If an empty token is passed we attempt to get the current token. If
// the token is not available we return nothing.
//
// If a successcallback or errorcallback handler is provided we make the call
// asynchronously and call them back. If none are provided we make the call synchronous and
// return the result.
//
function __getCampaignID (token, onsuccess, onnothing, onfailure)
{
    var gcurl = "https://api.emagazines.com/api/insight/getcampaigns/" + token;
    ...
    $.ajax({
        ...
        success: function(jsondata, textStatus, xhr) {
            //console.log("Got success: " + JSON.stringify(jsondata));
            if (onsuccess) {
                if (jsondata && (jsondata.length > 0)) {
                    var first = jsondata[0];
                    if (first) {
                        var campaign_id = first['campaign_id'];
                        var click_date = first['click_data'];
                        if (campaign_id && onsuccess) {
...
function EMGetCurrentCampaignID(successcallback, nothingcallback, errorcallback)
{
    var __e = __sharedEMA();
    __e.cache.getToken(function (token) {
        __getCampaignID(token, successcallback, nothingcallback, errorcallback);
    });
}

function EMGetCampaignID(token, successcallback, nothingcallback, errorcallback)
{
    var __e = __sharedEMA();
    if (token) {
        __getCampaignID(token, successcallback, nothingcallback, errorcallback);
    } else {
        __e.cache.getToken(function (t) {
            __getCampaignID(t, successcallback, nothingcallback, errorcallback);
        });
    }
}

/*
 * Calculate the recursive hash value of an object after serializing it into a string
 */
function _EMSerialize(object)
{
    var serialValue = "";
    ...
    if (objType === 'object') {
        var item;
        for (item in object) {
            serialValue += "[" + objType + ":" + item + _EMSerialize(object[item]) + "]";
        }
    } else if (objType === 'function') {
        serialValue += "[" + objType + ":" + object.toString() + "]";
    } else {
        serialValue += "[" + objType + ":" + object+"]";
    }
    return serialValue.replace(/\s/g, "");
}

function _EMHash(object)
{
    return MD5(_EMSerialize(object));
}
