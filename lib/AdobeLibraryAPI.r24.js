...
var TransactionState = {
	...
	toState: function(num) {
		switch(parseInt(num, 10)) {
			...
			case 0:
				return TransactionState.INITALIZED;
			case 100:
...
function Transaction() {
	 ...
	 * @memberOf adobeDPS-Transaction.prototype
	 * @default {adobeDPS-TransactionState#INITALIZED}
...
Transaction.prototype.start = function () {
	if (this.state != TransactionState.INITALIZED) {
	...
	if (json.hasOwnProperty("state")) {
		...
		if (this.state != newState) {
			...
			if (this.state == TransactionState.FINISHED ||
				...
				(this.state == TransactionState.FAILED && this.isFailureTerminal)) {
...
TransactionManager.prototype._update = function (data) {
	...
	if (data.hasOwnProperty("add")) {
		...
		if (changeList.length > 0) {
            ...
            Log.instance.warn("TransactionManager._update called with add that didn't add any transactions");
		}
        ...
        Log.instance.warn("TransactionManager._update cannot parse update: " + JSON.stringify(data));
	}
};
...
var FolioState = {
	...
	/** 
	 * <strong>Value: 200</strong>
	 * <br/> Folio is ready to be downloaded. 
	 */
	DOWNLOADABLE: 200,
	/** 
...
function Folio() {
	 ...
	 * used to determine whether {@link adobeDPS-Folio#view} may be called.
	 * @memberOf adobeDPS-Folio.prototype
	 * @type Boolean
	 */
	 ...
	 * (as opposed to the folio state) to determine whether {@link adobeDPS-Folio#archive}
	 * may be called.
	 * @memberOf adobeDPS-Folio.prototype
	 * @type Boolean
	 */
	 ...
	 * may call {@link adobeDPS-Folio#update} to update this folio.
	 * @memberOf adobeDPS-Folio.prototype
	 * @type Boolean
	 */
 ...
 * @throws {Error} If the Folio is in the wrong state or has another active transaction
 */
Folio.prototype.download = function () {
	if (this.state != FolioState.DOWNLOADABLE) {
		throw new Error('Attempting to download a folio with state: ' + this.state);
	}

    if(!this.isCompatible) {
        throw new Error('Attempting to download an incompatible folio.');
    }
...
/**
 * Start a view transaction on this Folio. Requires that {@link adobeDPS-Folio#isViewable}
 * be true.
 * @memberOf adobeDPS-Folio.prototype
 * @returns {adobeDPS-ViewTransaction} The started view transaction
 * @throws {Error} If {@link adobeDPS-Folio#isViewable} is false.
 */
Folio.prototype.view = function () {
	if (!this.isViewable) {
		throw new Error('Attempting to view a folio that is not viewable! You must check isViewable before attempting to view a folio.');
	}

    if(!this.isCompatible) {
        throw new Error('Attempting to view an incompatible folio.');
    }
...
/**
 * Start an update transaction on this Folio. Requires that {@link adobeDPS-Folio#isUpdatable}
 * be true.
 * @memberOf adobeDPS-Folio.prototype
 ...
 * @throws {Error} If the Folio has another active transaction
 */
Folio.prototype.getPreviewImage = function (width, height, isPortrait) {
...
/**
 * Update this folio object using a JSON message. This method should be called when the
 * Folio needs to be updated due to a model change on the native side.
 * @memberOf adobeDPS-Folio.prototype
 * @param {String} json The JSON message to process for updates
 */
Folio.prototype._updateFromJSON = function (json) {
	...
	for (prop in json) {
		if (this.hasOwnProperty(prop)) {
            ...
            Log.instance.warn("Found invalid property in folio update: " + prop + '-' + json[prop]);
		}
	}
...
LibraryService.prototype._update = function (data) {
	...
	var list, changesList, i, folioVO, folio, update, idEndIndex, folioId;
	if (data.hasOwnProperty("add")) {
		list = data.add;
        changesList = [];
		for (i = 0; i < list.length; i++) {
	...
	} else if (data.hasOwnProperty("update") && data.path.indexOf("folioMap") > -1) {
		update = data.update;
		idEndIndex = data.path.indexOf("']");
		// 10 is the number of characters in folioMap plus the [' before the id
		folioId = data.path.substring(10,idEndIndex);
		folio = this.folioMap.internal[folioId];
		
		if (folio) {
			folio._updateFromJSON(update);
		} else {
            Log.instance.warn('LibraryService._update called with unknown Folio: ' + folioId);
		}
 ...
 * @class Transaction associated with a Folio
...
function FolioTransaction(folio) {
	/**
	 * The Folio that this transaction is acting on
 ...
 * the currentTransactions array of the Folio associated with this transaction.
 * @override
 * @memberOf adobeDPS-FolioTransaction.prototype
...
/**
 * Callback when the transaction state change to handle association and dissociation with the Folio.
 * @memberOf adobeDPS-FolioTransaction.prototype
 * @private
...
FolioTransaction.prototype._updateFromJSON = function (json) {
	if (json.hasOwnProperty("folio")) {
		var folio = LibraryService.instance.folioMap.internal[json.folio];
		if (folio) {
			this.folio = folio;
 ...
 * @class Transaction associated with a Folio that changes the state of a folio. Only one of these may be acting on a folio at any given time.
 * @param {adobeDPS-Folio} folio The folio that this transaction is acting on
 ...
 * @throws {Error} If called when the Folio already has another FolioStateChangingTransaction acting on it.
 */
...
ApplicationState._fromJSON = function (json) {
	...
	this.buttonUpImage = json.upImage;
	this.buttonDownImage = json.downImage;
	this.buttonDisabledImage = json.disabledImage;
 ...
 * @class Transaction used to archive a Folio
 * @param {adobeDPS-Folio} folio The folio that this transaction is acting on
 ...
 * @class A Transaction that is used to download a Folio.
 * @param {adobeDPS-Folio} folio The folio that this transaction is acting on
 * @extends adobeDPS-FolioStateChangingTransaction
 * @throws {Error} If the folio passed in is invalid or not a Folio
 */
 ...
 * @class A Transaction that is used to download a Folio Preview Image.
 * @param {adobeDPS-Folio} folio The folio that this transaction is acting on
 ...
 * @class A Transaction that is used to view a Folio.
 * @extends adobeDPS-FolioTransaction
 */
 ...
 * @class A Transaction that is used to update a Folio.
 * @param {adobeDPS-Folio} folio The folio that this transaction is acting on
 * @extends adobeDPS-FolioStateChangingTransaction
 * @throws {Error} If the folio passed in is invalid or not a Folio
 */
			...
			for (i = 0; i < update.applicationStates.length; i++) {
                ...
                if(json.hasOwnProperty("type") && json.type == "ApplicationState") {
                    ...
                    if(state.type == ApplicationStateType.NATIVE || state.type == ApplicationStateType.WEBVIEW) {
                        this.applicationStates.push(state);
                    }
                }
			}
		}
...
DeviceService.prototype._initialize = function (data) {
	if (data.hasOwnProperty("update")) {
        ...
        if (update.hasOwnProperty("isPurchaseEnabled")) {
...
DeviceService.prototype._update = function (data) {
    if (data.hasOwnProperty("update")) {
        ...
        if (update.hasOwnProperty("isPurchaseEnabled") &&  this.isPurchasingEnabled != update.isPurchasingEnabled) {
            this.isPurchasingEnabled = update.isPurchasingEnabled;
            updates.push("isPurchaseEnabled");
        }
...
ReceiptService.prototype._initialize = function (data) {
    var update, adds, productId, vo, receipt, id, subscription;
	if (data.hasOwnProperty("update")) {
		update = data.update;
		adds = [];
		...
		if (update.hasOwnProperty("subscriptionReceipts")) {
			for (productId in update.subscriptionReceipts) {
				vo = update.subscriptionReceipts[productId];
				receipt = new SubscriptionReceipt(vo.broker, productId, vo.token, new Date(Number(vo.startDate)), (vo.endDate === null) ? null : new Date(Number(vo.endDate)));
				id = receipt.productId + receipt.startDate.getTime().toString();
...
ReceiptService.prototype._update = function (data) {
    var add, update, adds, i, vo, receipt, id, subscription, idEndIndex;
	if (data.hasOwnProperty("add")) {
		add = data.add;
		...
		for (i = 0; i < add.length; i++) {
			...
			if (vo.type == "Receipt") {
				...
				receipt = new SubscriptionReceipt(vo.broker, vo.productId, vo.token, new Date(Number(vo.startDate)), (vo.endDate === null) ? null : new Date(Number(vo.endDate)));
				id = receipt.productId + receipt.startDate.getTime().toString();
                subscription = this.availableSubscriptions[receipt.productId];
		...
		if (adds.length > 0) {
			this.newReceiptsAvailableSignal.dispatch(adds);
            Log.instance.debug('ReceiptService._update added ' + adds.length + ' receipts');
		} else {
            Log.instance.warn("ReceiptService._update called with an add that resulted in no new receipts");
		}
